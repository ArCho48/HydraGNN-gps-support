...
from hydragnn.models.embedder import FeatureEmbedder
...
class Base(Module):
    def __init__(
        ...
        self.feature_embedder = FeatureEmbedder(lpe_dim=self.lpe_dim,
                        ...
                        is_edge_model=self.is_edge_model)
    ...
    def loss_hpweighted(self, pred, value, head_index, var=None):
        ...
        for ihead in range(self.num_heads):
            ...
            if var is None:
                ...
                tot_loss += (
                    self.loss_function(head_pre[mask], head_val[mask]) * self.loss_weights[ihead]
                )
                tasks_loss.append(self.loss_function(head_pre[mask], head_val[mask]))
            else:
                head_var = var[ihead]
                tot_loss += (
                    self.loss_function(head_pre[mask], head_val[mask], head_var)
                    * self.loss_weights[ihead]
                )
                tasks_loss.append(self.loss_function(head_pre[mask], head_val[mask], head_var))
