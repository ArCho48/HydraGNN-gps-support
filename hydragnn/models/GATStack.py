...
class GATStack(Base):
    ...
    def _init_conv(self):
        ...
        if self.use_global_attn:
            self.graph_convs.append(self._apply_global_attn(self.get_conv(self.embed_dim, self.hidden_dim, concat=True, edge_dim=self.edge_embed_dim)))
            self.feature_layers.append(BatchNorm(self.hidden_dim))
            for _ in range(self.num_conv_layers - 2):
                self.graph_convs.append(self._apply_global_attn(self.get_conv(self.hidden_dim, self.hidden_dim, concat=True, edge_dim=self.edge_embed_dim)))
                self.feature_layers.append(BatchNorm(self.hidden_dim))
            self.graph_convs.append(self._apply_global_attn(self.get_conv(self.hidden_dim, self.hidden_dim, concat=False, edge_dim=self.edge_embed_dim)))
            self.feature_layers.append(BatchNorm(self.hidden_dim))
        else:
            self.graph_convs.append(self._apply_global_attn(self.get_conv(self.embed_dim, self.hidden_dim, concat=True, edge_dim=self.edge_embed_dim)))
            self.feature_layers.append(BatchNorm(self.hidden_dim * self.heads))
            for _ in range(self.num_conv_layers - 2):
                self.graph_convs.append(self._apply_global_attn(self.get_conv(self.hidden_dim * self.heads, self.hidden_dim, concat=True, edge_dim=self.edge_embed_dim)))
                self.feature_layers.append(BatchNorm(self.hidden_dim * self.heads))
            self.graph_convs.append(self._apply_global_attn(self.get_conv(self.hidden_dim * self.heads, self.hidden_dim, concat=False, edge_dim=self.edge_embed_dim)))
            self.feature_layers.append(BatchNorm(self.hidden_dim))
