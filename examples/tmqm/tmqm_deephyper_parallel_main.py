...
def run(trial, dequed=None):
    ...
    python_script = os.path.join(
        os.path.dirname(__file__), "tmqm_deephyper_parallel_trial.py"
    )

    # TODO: Launch a subprocess with `srun` to train neural networks
    params = trial.parameters
    log_name = "tmqm_hpo_trials" + "_" + str(trial.id)
...
if __name__ == "__main__":
    ...
    if config["NeuralNetwork"]["Architecture"]["global_attn_engine"]:
        if config["NeuralNetwork"]["Architecture"]["use_encodings"]:
            ...
            problem.add_hyperparameter([4,8,16,20,32], "hidden_dim")  # discrete parameter
        problem.add_hyperparameter([4,8,10,12], "edge_embed_dim")  # discrete parameter
    else:
