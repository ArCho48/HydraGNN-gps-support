...
def run(trial, dequed=None):
    ...
    python_script = os.path.join(
        os.path.dirname(__file__), "pcqm_deephyper_parallel_trial.py"
    )

    # TODO: Launch a subprocess with `srun` to train neural networks
    params = trial.parameters
    log_name = "pcqm_hpo_trials" + "_" + str(trial.id)
...
if __name__ == "__main__":
    ...
    if config["NeuralNetwork"]["Architecture"]["global_attn_engine"]:
        if config["NeuralNetwork"]["Architecture"]["use_encodings"]:
            ...
            problem.add_hyperparameter([2, 4, 8], "global_attn_heads")  # discrete parameter
            problem.add_hyperparameter([8,16,32,40,48], "hidden_dim")  # discrete parameter
        problem.add_hyperparameter([4,8,10,12,16], "edge_embed_dim")  # discrete parameter
    else:
        ...
        if config["NeuralNetwork"]["Architecture"]["use_encodings"]:
            ...
            problem.add_hyperparameter((8,40), "hidden_dim")  # discrete parameter
            problem.add_hyperparameter([0], "edge_embed_dim")  # discrete parameter

    problem.add_hyperparameter(["EGNN", "CGCNN", "PNA", "SchNet", "DimeNet", "PAINN"], "mpnn_type")  # categorical parameter
