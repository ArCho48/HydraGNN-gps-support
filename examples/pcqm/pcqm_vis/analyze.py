...
dataset = SimplePickleDataset(basedir='../dataset/pcqm.pickle', label="testset", var_config=var_config)
...
for G in top50:
    try: 
        measure = nx.eigenvector_centrality(G,max_iter=500)
    except:
        continue
    draw(G, nx.spring_layout(G, seed=675), measure, 'Eigenvector Centrality', 'eigens')

if not os.path.exists('comms'):
    os.makedirs('comms')
for G in top50: 
    try: 
        measure = nx.communicability_betweenness_centrality(G)
    except:
        continue
    draw(G, nx.spring_layout(G, seed=675), measure, 'Communicability Centrality', 'comms')
