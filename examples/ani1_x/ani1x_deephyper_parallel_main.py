...
def run(trial, dequed=None):
    ...
    python_script = os.path.join(
        os.path.dirname(__file__), "ani1x_deephyper_parallel_trial.py"
    )

    # TODO: Launch a subprocess with `srun` to train neural networks
    params = trial.parameters
    log_name = "ANI1x_hpo_trials" + "_" + str(trial.id)
...
if __name__ == "__main__":
    ...
    filename = os.path.join(os.path.dirname(os.path.abspath(__file__)), "ani1x_energy.json")
