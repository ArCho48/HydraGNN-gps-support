...
def run(trial, dequed=None):
    ...
    python_script = os.path.join(
        os.path.dirname(__file__), "transition1x_deephyper_parallel_trial.py"
    )

    # TODO: Launch a subprocess with `srun` to train neural networks
    params = trial.parameters
    log_name = "transition1x_hpo_trials" + "_" + str(trial.id)
...
if __name__ == "__main__":
    ...
    log_name = "transition1x_hpo_trials"
    ...
    filename = os.path.join(os.path.dirname(os.path.abspath(__file__)), "transition1x_energy.json")
    ...
    if config["NeuralNetwork"]["Architecture"]["global_attn_engine"]:
        ...
        problem.add_hyperparameter([8,12,16,32], "edge_embed_dim")  # discrete parameter
    else:
