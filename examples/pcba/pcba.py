...
class PCBA_enc(AbstractBaseDataset):
    def __init__(
        ...
        for data in tqdm(train, total=len(train), desc="Train"):
            # node features: {'atomic_num', 'chirality', 'degree', 'formal_charge', 
            #   'numH', 'number_radical_e', 'hybridization', 'is_aromatic', 'is_in_ring'}
            # edge features: {'bond_type', 'bond_stereo', 'is_conjugated'}
            data.x = data.x.to(torch.float32)
            data.y = data.y.to(torch.float32) #groundtruth has NaNs as not every molecule was tested for every class
...
def main(preonly=False, format='pickle', ddstore=False, 
    ...
    if preonly:      
        # Raw data 
        dataset = PygGraphPropPredDataset(name="ogbg-molpcba",root='dataset/raw') 
    ...
    log_name = f"pcba_test_{mpnn_type}" if mpnn_type else "pcba_test"
    ...
    # Run training with the given model and pcba datasets.
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(
        description="Run the pcba example with optional model type."
    )
    parser.add_argument(
        "--preonly",
        action="store_true",
